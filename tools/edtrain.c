...
int main(int argc,char **argv){
    ...
    if(!dataset_path||!output_path){fprintf(stderr,"usage: edtrain --dataset D.edb [--selection-dataset V.edb] [--weights models/picodet_s_coco80_fp16.edm] --output O.edm [--output-precision fp32|fp16|int8|int4] [--head-adapter independent|cross-level] [--cross-level-mix F] [--roi-head] [--picofeat-adapter] [--quality-adapter] [--nesterov] [--aligned-loss] [--budget-ms 50000] [--threads N] [--seed N] [--learning-rate LR] [--mosaic-size 0|1|4] [--accumulation N] [--train-scope outputs|classification|quality] [--feature-cache-samples N] [--max-steps N] [--restart-after-steps N] [--sample-mode auto|full|zoom|mixed|mosaic|tile|adapt] [--calibration-samples N] [--init-mode auto|single|equal|ap-weighted|prototype|proto-residual] [--proto-solver ridge|diag|lda] [--class-lr off|auto] [--class-loss-norm] [--hard-neg] [--zero-update] [--no-class-calibration] [--dfl-lr-scale F] [--ema-decay F] [--wise-mix F]\n");return 2;}
    ...
    if(guard_model){ed_map_report baseline_map,adapted_map;ed_model *serialized=NULL;uint32_t ci;int regressed=0;
        ...
        if(status==ED_OK)status=ed_evaluate_map50(guard_model,selection,0.001f,0.42f,&baseline_map);if(status==ED_OK)status=ed_evaluate_map50(serialized,selection,0.001f,0.42f,&adapted_map);
        if(status!=ED_OK){fprintf(stderr,"selection eval: %s\n",ed_status_string(status));goto fail;}
        if(adapted_map.map50+1e-7f<baseline_map.map50)regressed=1;
        for(ci=0;ci<baseline_map.class_count;++ci)if(adapted_map.per_class_ap[ci]+1e-7f<baseline_map.per_class_ap[ci]){regressed=1;printf("selection_class=%s baseline_ap50=%.6f adapted_ap50=%.6f regressed=1\n",ed_model_class_name(guard_model,ci),baseline_map.per_class_ap[ci],adapted_map.per_class_ap[ci]);}
        if(regressed){ed_model_free(serialized);ed_model_free(model);model=guard_model;guard_model=NULL;printf("selection_baseline_map50=%.6f selection_adapted_map50=%.6f selection_decision=baseline\n",baseline_map.map50,adapted_map.map50);}
        else{ed_model_free(model);model=serialized;serialized=NULL;output_saved=1;printf("selection_baseline_map50=%.6f selection_adapted_map50=%.6f selection_decision=adapted\n",baseline_map.map50,adapted_map.map50);}
    }
    ...
    ed_dataset_free(selection);ed_dataset_free(dataset);ed_model_free(guard_model);ed_model_free(model);return report.deadline_respected?0:1;
fail:ed_dataset_free(selection);ed_dataset_free(dataset);ed_model_free(guard_model);ed_model_free(model);return 1;
}
