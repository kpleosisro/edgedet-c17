...
int main(int argc,char **argv){
    const char *dataset_path=arg(argc,argv,"--dataset"),*selection_path=arg(argc,argv,"--selection-dataset"),*selection_score_text=arg(argc,argv,"--selection-score-threshold"),*weights_path=arg(argc,argv,"--weights"),*output_path=arg(argc,argv,"--output"),*storage_text=arg(argc,argv,"--output-precision"),*adapter_text=arg(argc,argv,"--head-adapter"),*cross_mix_text=arg(argc,argv,"--cross-level-mix"),*budget_text=arg(argc,argv,"--budget-ms"),*threads_text=arg(argc,argv,"--threads"),*seed_text=arg(argc,argv,"--seed"),*lr_text=arg(argc,argv,"--learning-rate"),*mosaic_text=arg(argc,argv,"--mosaic-size"),*accumulation_text=arg(argc,argv,"--accumulation"),*scope_text=arg(argc,argv,"--train-scope"),*cache_text=arg(argc,argv,"--feature-cache-samples"),*max_steps_text=arg(argc,argv,"--max-steps"),*restart_text=arg(argc,argv,"--restart-after-steps"),*sample_text=arg(argc,argv,"--sample-mode"),*ema_text=arg(argc,argv,"--ema-decay"),*wise_text=arg(argc,argv,"--wise-mix"),*dfl_text=arg(argc,argv,"--dfl-lr-scale"),*calibration_text=arg(argc,argv,"--calibration-samples"),*init_text=arg(argc,argv,"--init-mode"),*solver_text=arg(argc,argv,"--proto-solver"),*class_lr_text=arg(argc,argv,"--class-lr"),*runtime_text=arg(argc,argv,"--runtime");
    ...
    if(runtime_text){if(strcmp(runtime_text,"fpga-model")==0){if(ed_runtime_set_compute(ED_COMPUTE_FPGA_MODEL)!=ED_OK)return 2;}
        else if(strcmp(runtime_text,"host")!=0){fprintf(stderr,"invalid --runtime (expected host or fpga-model)\n");return 2;}}
