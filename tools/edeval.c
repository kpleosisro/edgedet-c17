...
int main(int argc,char**argv){
    ...
    uint32_t tiles_x=1u,tiles_y=1u;int include_full=has_flag(argc,argv,"--include-full");
    ...
    if(!dp||!mp){fprintf(stderr,"usage: edeval --dataset D.edb --model M.edm [--threads N] [--score-threshold 0.025] [--nms-threshold 0.6] [--tiles-x auto|N --tiles-y N --tile-overlap F --include-full] [--soft-nms SIGMA|linear] [--dump-detections FILE.csv]\n");return 2;}
    ...
    s=ed_evaluate_map50_ex(m,d,threshold,nms,tiles_x,tiles_y,tile_overlap,include_full,soft_nms,&r);
