...
static ed_status evaluate_map50_impl(const ed_model*m,const ed_dataset*d,float st,float nt,uint32_t tiles_x,uint32_t tiles_y,float tile_overlap,int include_full,float soft_nms_sigma,ed_map_report*r){
    ...
    for(image_index=0;image_index<d->header.record_count;++image_index){
        ...
        {float predict_nt=soft_nms_sigma>0.0f?1.0f:nt;
            ...
            else if(tiles_x*tiles_y>1u)status=ed_predict_tiled(m,&image,st,predict_nt,tiles_x,tiles_y,tile_overlap,include_full,&list);
    ...
    for(c=0;c<m->class_count;++c){
        ...
        for(k=0;k<prediction_count;++k){
            ...
            for(j=0;j<record->annotation_count;++j){float iou=box_iou(p->detection.x1,p->detection.y1,p->detection.x2,p->detection.y2,anns[j].x1,anns[j].y1,anns[j].x2,anns[j].y2);
                ...
                if(!(anns[j].flags&ED_ANN_FLAG_IGNORE)&&anns[j].class_id==c&&iou>best_iou){best_iou=iou;best=j;}
            }
            if(best_iou>=0.5f&&best!=UINT32_MAX&&!matched[p->image_index][best]){matched[p->image_index][best]=1u;++tp;}
...
ed_status ed_evaluate_map50_ex(const ed_model*m,const ed_dataset*d,float st,float nt,uint32_t tiles_x,uint32_t tiles_y,float tile_overlap,int include_full,float soft_nms_sigma,ed_map_report*r){return evaluate_map50_impl(m,d,st,nt,tiles_x,tiles_y,tile_overlap,include_full,soft_nms_sigma,r);}
ed_status ed_evaluate_map50_tiled(const ed_model*m,const ed_dataset*d,float st,float nt,uint32_t tiles_x,uint32_t tiles_y,float tile_overlap,int include_full,ed_map_report*r){return evaluate_map50_impl(m,d,st,nt,tiles_x,tiles_y,tile_overlap,include_full,0.0f,r);}
ed_status ed_evaluate_map50(const ed_model*m,const ed_dataset*d,float st,float nt,ed_map_report*r){return evaluate_map50_impl(m,d,st,nt,1u,1u,0.0f,0,0.0f,r);}
